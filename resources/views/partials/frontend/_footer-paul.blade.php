<!-- Footer -->
<footer>
    <div class="parallax100 p-t-35 p-b-15 bg-1">
        <div class="container">
            <div class="row justify-content-center">
                <div class="col-sm-12 col-md-12 col-lg-12 justify-content-center p-b-20 m-b-20 bottom-line-white">
                    <div class="col-md-6 col-6 text-center text-md-right">
                        <span class="t1-m-1 text-light">Part of</span>
                    </div>
                    <div class="col-md-6 col-6 text-center text-md-left">
                        <img src="{{ asset('images/paulmarc/introduction/Marcs Website (Paul) components-36.png') }}" height="20" style="margin-top: -28px;">
                    </div>
                </div>

                <div class="col-sm-8 col-md-5 col-lg-3 p-b-20">
                    <div class="size-h-1 flex-s-e p-b-6 m-b-18">
                        <a href="#">
                            <img class="max-s-full" src="{{ asset('images/marc/about/Marcs Website components-19.png') }}" alt="IMG" style="max-width: 200px;">
                        </a>
                    </div>

                    <div>
                        <p class="t1-s-2 cl-13 p-b-9">
                            PT PURI TRINITI BATAM
                        </p>
                    </div>
                </div>

                <div class="col-sm-8 col-md-5 col-lg-3 p-b-20">
                    <div class="size-h-1 flex-s-e m-b-18">
                        <h4 class="t1-m-3 text-uppercase cl-0">
                            Marketing Office
                        </h4>
                    </div>

                    <ul>
                        <li class="flex-wr-s-s t1-s-2 cl-13 p-b-9">
                            <span class="size-w-4">
                                Komp. Ruko Batamas Blok A No. 1
                                Pasir Putih - Batam Centre
                                021 80821 403 / 80821 333
                            </span>
                        </li>
                    </ul>
                </div>

                <div class="col-sm-8 col-md-5 col-lg-3 p-b-20">
                    <div class="size-h-1 flex-s-e m-b-18">
                        <h4 class="t1-m-3 text-uppercase cl-0">
                            Triniti Land HQ
                        </h4>
                    </div>

                    <ul>
                        <li class="flex-wr-s-s t1-s-2 cl-13 p-b-9">
                            <span class="size-w-4">
                                Brooklyn Premium Office no. 1-3
                                Jl. Sutera Boulevard Kav. 22-26
                                Alam Sutera - Tangerang
                            </span>
                        </li>
                    </ul>
                </div>

                <div class="col-sm-8 col-md-5 col-lg-3 p-b-20">
                    <div class="size-h-1 flex-s-e m-b-18">
                        <h4 class="t1-m-3 text-uppercase cl-0">
                            Contact
                        </h4>
                    </div>

                    <ul>
                        <li class="flex-wr-s-s t1-s-2 cl-13 p-b-9">
                            <span class="size-w-4">
                               (+62) 300 666 88
                                info@trinitiland.com
                                www.trinitiland.com
                            </span>
                        </li>
                    </ul>
                </div>
            </div>
        </div>
    </div>
</footer>